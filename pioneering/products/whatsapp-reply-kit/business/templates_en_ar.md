# WhatsApp Reply Kit - Business

## 001. Pricing Inquiry
EN: Thanks for your message. The price for [SERVICE] starts from [PRICE] BHD.
AR: شكرًا لرسالتك. سعر [SERVICE] يبدأ من [PRICE] د.ب.

## 002. Discount Request
EN: I can offer [DISCOUNT]% for confirmation today.
AR: أقدر أقدم خصم [DISCOUNT]% في حال التأكيد اليوم.

## 003. Follow-up
EN: Just checking in 👋 Did you get a chance to review the quote?
AR: مجرد متابعة سريعة 👋 هل قدرت تراجع عرض السعر؟

## 004. Payment Reminder
EN: Friendly reminder: invoice #[INV_NO] of [AMOUNT] BHD is due on [DATE].
AR: تذكير ودي: الفاتورة #[INV_NO] بقيمة [AMOUNT] د.ب مستحقة بتاريخ [DATE].

## 005. Complaint Handling
EN: Sorry for the inconvenience. I will resolve this by [TIME].
AR: نعتذر عن الإزعاج، وسيتم حل الموضوع قبل [TIME].

## 006. Appointment Confirmation
EN: Confirmed ✅ Your booking is on [DATE] at [TIME].
AR: تم التأكيد ✅ حجزك يوم [DATE] الساعة [TIME].

## 007. Reschedule
EN: No problem, we can move your booking to [NEW_DATE] at [NEW_TIME].
AR: أكيد، نقدر نغير الحجز إلى [NEW_DATE] الساعة [NEW_TIME].

## 008. Out of Stock
EN: This item is currently out of stock. Expected availability: [DATE].
AR: المنتج غير متوفر حاليًا. التوفر المتوقع: [DATE].

## 009. Upsell
EN: You may also like [ADDON] for an extra [ADDON_PRICE] BHD.
AR: ممكن يناسبك أيضًا [ADDON] مقابل [ADDON_PRICE] د.ب إضافية.

## 010. Closing
EN: Perfect, I’ll proceed now and keep you updated.
AR: ممتاز، سأبدأ الآن وأوافيك بالتحديثات.

## 011. Pricing Inquiry
EN: Thanks for your message. The price for [SERVICE] starts from [PRICE] BHD.
AR: شكرًا لرسالتك. سعر [SERVICE] يبدأ من [PRICE] د.ب.

## 012. Discount Request
EN: I can offer [DISCOUNT]% for confirmation today.
AR: أقدر أقدم خصم [DISCOUNT]% في حال التأكيد اليوم.

## 013. Follow-up
EN: Just checking in 👋 Did you get a chance to review the quote?
AR: مجرد متابعة سريعة 👋 هل قدرت تراجع عرض السعر؟

## 014. Payment Reminder
EN: Friendly reminder: invoice #[INV_NO] of [AMOUNT] BHD is due on [DATE].
AR: تذكير ودي: الفاتورة #[INV_NO] بقيمة [AMOUNT] د.ب مستحقة بتاريخ [DATE].

## 015. Complaint Handling
EN: Sorry for the inconvenience. I will resolve this by [TIME].
AR: نعتذر عن الإزعاج، وسيتم حل الموضوع قبل [TIME].

## 016. Appointment Confirmation
EN: Confirmed ✅ Your booking is on [DATE] at [TIME].
AR: تم التأكيد ✅ حجزك يوم [DATE] الساعة [TIME].

## 017. Reschedule
EN: No problem, we can move your booking to [NEW_DATE] at [NEW_TIME].
AR: أكيد، نقدر نغير الحجز إلى [NEW_DATE] الساعة [NEW_TIME].

## 018. Out of Stock
EN: This item is currently out of stock. Expected availability: [DATE].
AR: المنتج غير متوفر حاليًا. التوفر المتوقع: [DATE].

## 019. Upsell
EN: You may also like [ADDON] for an extra [ADDON_PRICE] BHD.
AR: ممكن يناسبك أيضًا [ADDON] مقابل [ADDON_PRICE] د.ب إضافية.

## 020. Closing
EN: Perfect, I’ll proceed now and keep you updated.
AR: ممتاز، سأبدأ الآن وأوافيك بالتحديثات.

## 021. Pricing Inquiry
EN: Thanks for your message. The price for [SERVICE] starts from [PRICE] BHD.
AR: شكرًا لرسالتك. سعر [SERVICE] يبدأ من [PRICE] د.ب.

## 022. Discount Request
EN: I can offer [DISCOUNT]% for confirmation today.
AR: أقدر أقدم خصم [DISCOUNT]% في حال التأكيد اليوم.

## 023. Follow-up
EN: Just checking in 👋 Did you get a chance to review the quote?
AR: مجرد متابعة سريعة 👋 هل قدرت تراجع عرض السعر؟

## 024. Payment Reminder
EN: Friendly reminder: invoice #[INV_NO] of [AMOUNT] BHD is due on [DATE].
AR: تذكير ودي: الفاتورة #[INV_NO] بقيمة [AMOUNT] د.ب مستحقة بتاريخ [DATE].

## 025. Complaint Handling
EN: Sorry for the inconvenience. I will resolve this by [TIME].
AR: نعتذر عن الإزعاج، وسيتم حل الموضوع قبل [TIME].

## 026. Appointment Confirmation
EN: Confirmed ✅ Your booking is on [DATE] at [TIME].
AR: تم التأكيد ✅ حجزك يوم [DATE] الساعة [TIME].

## 027. Reschedule
EN: No problem, we can move your booking to [NEW_DATE] at [NEW_TIME].
AR: أكيد، نقدر نغير الحجز إلى [NEW_DATE] الساعة [NEW_TIME].

## 028. Out of Stock
EN: This item is currently out of stock. Expected availability: [DATE].
AR: المنتج غير متوفر حاليًا. التوفر المتوقع: [DATE].

## 029. Upsell
EN: You may also like [ADDON] for an extra [ADDON_PRICE] BHD.
AR: ممكن يناسبك أيضًا [ADDON] مقابل [ADDON_PRICE] د.ب إضافية.

## 030. Closing
EN: Perfect, I’ll proceed now and keep you updated.
AR: ممتاز، سأبدأ الآن وأوافيك بالتحديثات.

## 031. Pricing Inquiry
EN: Thanks for your message. The price for [SERVICE] starts from [PRICE] BHD.
AR: شكرًا لرسالتك. سعر [SERVICE] يبدأ من [PRICE] د.ب.

## 032. Discount Request
EN: I can offer [DISCOUNT]% for confirmation today.
AR: أقدر أقدم خصم [DISCOUNT]% في حال التأكيد اليوم.

## 033. Follow-up
EN: Just checking in 👋 Did you get a chance to review the quote?
AR: مجرد متابعة سريعة 👋 هل قدرت تراجع عرض السعر؟

## 034. Payment Reminder
EN: Friendly reminder: invoice #[INV_NO] of [AMOUNT] BHD is due on [DATE].
AR: تذكير ودي: الفاتورة #[INV_NO] بقيمة [AMOUNT] د.ب مستحقة بتاريخ [DATE].

## 035. Complaint Handling
EN: Sorry for the inconvenience. I will resolve this by [TIME].
AR: نعتذر عن الإزعاج، وسيتم حل الموضوع قبل [TIME].

## 036. Appointment Confirmation
EN: Confirmed ✅ Your booking is on [DATE] at [TIME].
AR: تم التأكيد ✅ حجزك يوم [DATE] الساعة [TIME].

## 037. Reschedule
EN: No problem, we can move your booking to [NEW_DATE] at [NEW_TIME].
AR: أكيد، نقدر نغير الحجز إلى [NEW_DATE] الساعة [NEW_TIME].

## 038. Out of Stock
EN: This item is currently out of stock. Expected availability: [DATE].
AR: المنتج غير متوفر حاليًا. التوفر المتوقع: [DATE].

## 039. Upsell
EN: You may also like [ADDON] for an extra [ADDON_PRICE] BHD.
AR: ممكن يناسبك أيضًا [ADDON] مقابل [ADDON_PRICE] د.ب إضافية.

## 040. Closing
EN: Perfect, I’ll proceed now and keep you updated.
AR: ممتاز، سأبدأ الآن وأوافيك بالتحديثات.

## 041. Pricing Inquiry
EN: Thanks for your message. The price for [SERVICE] starts from [PRICE] BHD.
AR: شكرًا لرسالتك. سعر [SERVICE] يبدأ من [PRICE] د.ب.

## 042. Discount Request
EN: I can offer [DISCOUNT]% for confirmation today.
AR: أقدر أقدم خصم [DISCOUNT]% في حال التأكيد اليوم.

## 043. Follow-up
EN: Just checking in 👋 Did you get a chance to review the quote?
AR: مجرد متابعة سريعة 👋 هل قدرت تراجع عرض السعر؟

## 044. Payment Reminder
EN: Friendly reminder: invoice #[INV_NO] of [AMOUNT] BHD is due on [DATE].
AR: تذكير ودي: الفاتورة #[INV_NO] بقيمة [AMOUNT] د.ب مستحقة بتاريخ [DATE].

## 045. Complaint Handling
EN: Sorry for the inconvenience. I will resolve this by [TIME].
AR: نعتذر عن الإزعاج، وسيتم حل الموضوع قبل [TIME].

## 046. Appointment Confirmation
EN: Confirmed ✅ Your booking is on [DATE] at [TIME].
AR: تم التأكيد ✅ حجزك يوم [DATE] الساعة [TIME].

## 047. Reschedule
EN: No problem, we can move your booking to [NEW_DATE] at [NEW_TIME].
AR: أكيد، نقدر نغير الحجز إلى [NEW_DATE] الساعة [NEW_TIME].

## 048. Out of Stock
EN: This item is currently out of stock. Expected availability: [DATE].
AR: المنتج غير متوفر حاليًا. التوفر المتوقع: [DATE].

## 049. Upsell
EN: You may also like [ADDON] for an extra [ADDON_PRICE] BHD.
AR: ممكن يناسبك أيضًا [ADDON] مقابل [ADDON_PRICE] د.ب إضافية.

## 050. Closing
EN: Perfect, I’ll proceed now and keep you updated.
AR: ممتاز، سأبدأ الآن وأوافيك بالتحديثات.

## 051. Pricing Inquiry
EN: Thanks for your message. The price for [SERVICE] starts from [PRICE] BHD.
AR: شكرًا لرسالتك. سعر [SERVICE] يبدأ من [PRICE] د.ب.

## 052. Discount Request
EN: I can offer [DISCOUNT]% for confirmation today.
AR: أقدر أقدم خصم [DISCOUNT]% في حال التأكيد اليوم.

## 053. Follow-up
EN: Just checking in 👋 Did you get a chance to review the quote?
AR: مجرد متابعة سريعة 👋 هل قدرت تراجع عرض السعر؟

## 054. Payment Reminder
EN: Friendly reminder: invoice #[INV_NO] of [AMOUNT] BHD is due on [DATE].
AR: تذكير ودي: الفاتورة #[INV_NO] بقيمة [AMOUNT] د.ب مستحقة بتاريخ [DATE].

## 055. Complaint Handling
EN: Sorry for the inconvenience. I will resolve this by [TIME].
AR: نعتذر عن الإزعاج، وسيتم حل الموضوع قبل [TIME].

## 056. Appointment Confirmation
EN: Confirmed ✅ Your booking is on [DATE] at [TIME].
AR: تم التأكيد ✅ حجزك يوم [DATE] الساعة [TIME].

## 057. Reschedule
EN: No problem, we can move your booking to [NEW_DATE] at [NEW_TIME].
AR: أكيد، نقدر نغير الحجز إلى [NEW_DATE] الساعة [NEW_TIME].

## 058. Out of Stock
EN: This item is currently out of stock. Expected availability: [DATE].
AR: المنتج غير متوفر حاليًا. التوفر المتوقع: [DATE].

## 059. Upsell
EN: You may also like [ADDON] for an extra [ADDON_PRICE] BHD.
AR: ممكن يناسبك أيضًا [ADDON] مقابل [ADDON_PRICE] د.ب إضافية.

## 060. Closing
EN: Perfect, I’ll proceed now and keep you updated.
AR: ممتاز، سأبدأ الآن وأوافيك بالتحديثات.

## 061. Pricing Inquiry
EN: Thanks for your message. The price for [SERVICE] starts from [PRICE] BHD.
AR: شكرًا لرسالتك. سعر [SERVICE] يبدأ من [PRICE] د.ب.

## 062. Discount Request
EN: I can offer [DISCOUNT]% for confirmation today.
AR: أقدر أقدم خصم [DISCOUNT]% في حال التأكيد اليوم.

## 063. Follow-up
EN: Just checking in 👋 Did you get a chance to review the quote?
AR: مجرد متابعة سريعة 👋 هل قدرت تراجع عرض السعر؟

## 064. Payment Reminder
EN: Friendly reminder: invoice #[INV_NO] of [AMOUNT] BHD is due on [DATE].
AR: تذكير ودي: الفاتورة #[INV_NO] بقيمة [AMOUNT] د.ب مستحقة بتاريخ [DATE].

## 065. Complaint Handling
EN: Sorry for the inconvenience. I will resolve this by [TIME].
AR: نعتذر عن الإزعاج، وسيتم حل الموضوع قبل [TIME].

## 066. Appointment Confirmation
EN: Confirmed ✅ Your booking is on [DATE] at [TIME].
AR: تم التأكيد ✅ حجزك يوم [DATE] الساعة [TIME].

## 067. Reschedule
EN: No problem, we can move your booking to [NEW_DATE] at [NEW_TIME].
AR: أكيد، نقدر نغير الحجز إلى [NEW_DATE] الساعة [NEW_TIME].

## 068. Out of Stock
EN: This item is currently out of stock. Expected availability: [DATE].
AR: المنتج غير متوفر حاليًا. التوفر المتوقع: [DATE].

## 069. Upsell
EN: You may also like [ADDON] for an extra [ADDON_PRICE] BHD.
AR: ممكن يناسبك أيضًا [ADDON] مقابل [ADDON_PRICE] د.ب إضافية.

## 070. Closing
EN: Perfect, I’ll proceed now and keep you updated.
AR: ممتاز، سأبدأ الآن وأوافيك بالتحديثات.

## 071. Pricing Inquiry
EN: Thanks for your message. The price for [SERVICE] starts from [PRICE] BHD.
AR: شكرًا لرسالتك. سعر [SERVICE] يبدأ من [PRICE] د.ب.

## 072. Discount Request
EN: I can offer [DISCOUNT]% for confirmation today.
AR: أقدر أقدم خصم [DISCOUNT]% في حال التأكيد اليوم.

## 073. Follow-up
EN: Just checking in 👋 Did you get a chance to review the quote?
AR: مجرد متابعة سريعة 👋 هل قدرت تراجع عرض السعر؟

## 074. Payment Reminder
EN: Friendly reminder: invoice #[INV_NO] of [AMOUNT] BHD is due on [DATE].
AR: تذكير ودي: الفاتورة #[INV_NO] بقيمة [AMOUNT] د.ب مستحقة بتاريخ [DATE].

## 075. Complaint Handling
EN: Sorry for the inconvenience. I will resolve this by [TIME].
AR: نعتذر عن الإزعاج، وسيتم حل الموضوع قبل [TIME].

## 076. Appointment Confirmation
EN: Confirmed ✅ Your booking is on [DATE] at [TIME].
AR: تم التأكيد ✅ حجزك يوم [DATE] الساعة [TIME].

## 077. Reschedule
EN: No problem, we can move your booking to [NEW_DATE] at [NEW_TIME].
AR: أكيد، نقدر نغير الحجز إلى [NEW_DATE] الساعة [NEW_TIME].

## 078. Out of Stock
EN: This item is currently out of stock. Expected availability: [DATE].
AR: المنتج غير متوفر حاليًا. التوفر المتوقع: [DATE].

## 079. Upsell
EN: You may also like [ADDON] for an extra [ADDON_PRICE] BHD.
AR: ممكن يناسبك أيضًا [ADDON] مقابل [ADDON_PRICE] د.ب إضافية.

## 080. Closing
EN: Perfect, I’ll proceed now and keep you updated.
AR: ممتاز، سأبدأ الآن وأوافيك بالتحديثات.

## 081. Pricing Inquiry
EN: Thanks for your message. The price for [SERVICE] starts from [PRICE] BHD.
AR: شكرًا لرسالتك. سعر [SERVICE] يبدأ من [PRICE] د.ب.

## 082. Discount Request
EN: I can offer [DISCOUNT]% for confirmation today.
AR: أقدر أقدم خصم [DISCOUNT]% في حال التأكيد اليوم.

## 083. Follow-up
EN: Just checking in 👋 Did you get a chance to review the quote?
AR: مجرد متابعة سريعة 👋 هل قدرت تراجع عرض السعر؟

## 084. Payment Reminder
EN: Friendly reminder: invoice #[INV_NO] of [AMOUNT] BHD is due on [DATE].
AR: تذكير ودي: الفاتورة #[INV_NO] بقيمة [AMOUNT] د.ب مستحقة بتاريخ [DATE].

## 085. Complaint Handling
EN: Sorry for the inconvenience. I will resolve this by [TIME].
AR: نعتذر عن الإزعاج، وسيتم حل الموضوع قبل [TIME].

## 086. Appointment Confirmation
EN: Confirmed ✅ Your booking is on [DATE] at [TIME].
AR: تم التأكيد ✅ حجزك يوم [DATE] الساعة [TIME].

## 087. Reschedule
EN: No problem, we can move your booking to [NEW_DATE] at [NEW_TIME].
AR: أكيد، نقدر نغير الحجز إلى [NEW_DATE] الساعة [NEW_TIME].

## 088. Out of Stock
EN: This item is currently out of stock. Expected availability: [DATE].
AR: المنتج غير متوفر حاليًا. التوفر المتوقع: [DATE].

## 089. Upsell
EN: You may also like [ADDON] for an extra [ADDON_PRICE] BHD.
AR: ممكن يناسبك أيضًا [ADDON] مقابل [ADDON_PRICE] د.ب إضافية.

## 090. Closing
EN: Perfect, I’ll proceed now and keep you updated.
AR: ممتاز، سأبدأ الآن وأوافيك بالتحديثات.

## 091. Pricing Inquiry
EN: Thanks for your message. The price for [SERVICE] starts from [PRICE] BHD.
AR: شكرًا لرسالتك. سعر [SERVICE] يبدأ من [PRICE] د.ب.

## 092. Discount Request
EN: I can offer [DISCOUNT]% for confirmation today.
AR: أقدر أقدم خصم [DISCOUNT]% في حال التأكيد اليوم.

## 093. Follow-up
EN: Just checking in 👋 Did you get a chance to review the quote?
AR: مجرد متابعة سريعة 👋 هل قدرت تراجع عرض السعر؟

## 094. Payment Reminder
EN: Friendly reminder: invoice #[INV_NO] of [AMOUNT] BHD is due on [DATE].
AR: تذكير ودي: الفاتورة #[INV_NO] بقيمة [AMOUNT] د.ب مستحقة بتاريخ [DATE].

## 095. Complaint Handling
EN: Sorry for the inconvenience. I will resolve this by [TIME].
AR: نعتذر عن الإزعاج، وسيتم حل الموضوع قبل [TIME].

## 096. Appointment Confirmation
EN: Confirmed ✅ Your booking is on [DATE] at [TIME].
AR: تم التأكيد ✅ حجزك يوم [DATE] الساعة [TIME].

## 097. Reschedule
EN: No problem, we can move your booking to [NEW_DATE] at [NEW_TIME].
AR: أكيد، نقدر نغير الحجز إلى [NEW_DATE] الساعة [NEW_TIME].

## 098. Out of Stock
EN: This item is currently out of stock. Expected availability: [DATE].
AR: المنتج غير متوفر حاليًا. التوفر المتوقع: [DATE].

## 099. Upsell
EN: You may also like [ADDON] for an extra [ADDON_PRICE] BHD.
AR: ممكن يناسبك أيضًا [ADDON] مقابل [ADDON_PRICE] د.ب إضافية.

## 100. Closing
EN: Perfect, I’ll proceed now and keep you updated.
AR: ممتاز، سأبدأ الآن وأوافيك بالتحديثات.

## 101. Pricing Inquiry
EN: Thanks for your message. The price for [SERVICE] starts from [PRICE] BHD.
AR: شكرًا لرسالتك. سعر [SERVICE] يبدأ من [PRICE] د.ب.

## 102. Discount Request
EN: I can offer [DISCOUNT]% for confirmation today.
AR: أقدر أقدم خصم [DISCOUNT]% في حال التأكيد اليوم.

## 103. Follow-up
EN: Just checking in 👋 Did you get a chance to review the quote?
AR: مجرد متابعة سريعة 👋 هل قدرت تراجع عرض السعر؟

## 104. Payment Reminder
EN: Friendly reminder: invoice #[INV_NO] of [AMOUNT] BHD is due on [DATE].
AR: تذكير ودي: الفاتورة #[INV_NO] بقيمة [AMOUNT] د.ب مستحقة بتاريخ [DATE].

## 105. Complaint Handling
EN: Sorry for the inconvenience. I will resolve this by [TIME].
AR: نعتذر عن الإزعاج، وسيتم حل الموضوع قبل [TIME].

## 106. Appointment Confirmation
EN: Confirmed ✅ Your booking is on [DATE] at [TIME].
AR: تم التأكيد ✅ حجزك يوم [DATE] الساعة [TIME].

## 107. Reschedule
EN: No problem, we can move your booking to [NEW_DATE] at [NEW_TIME].
AR: أكيد، نقدر نغير الحجز إلى [NEW_DATE] الساعة [NEW_TIME].

## 108. Out of Stock
EN: This item is currently out of stock. Expected availability: [DATE].
AR: المنتج غير متوفر حاليًا. التوفر المتوقع: [DATE].

## 109. Upsell
EN: You may also like [ADDON] for an extra [ADDON_PRICE] BHD.
AR: ممكن يناسبك أيضًا [ADDON] مقابل [ADDON_PRICE] د.ب إضافية.

## 110. Closing
EN: Perfect, I’ll proceed now and keep you updated.
AR: ممتاز، سأبدأ الآن وأوافيك بالتحديثات.

## 111. Pricing Inquiry
EN: Thanks for your message. The price for [SERVICE] starts from [PRICE] BHD.
AR: شكرًا لرسالتك. سعر [SERVICE] يبدأ من [PRICE] د.ب.

## 112. Discount Request
EN: I can offer [DISCOUNT]% for confirmation today.
AR: أقدر أقدم خصم [DISCOUNT]% في حال التأكيد اليوم.

## 113. Follow-up
EN: Just checking in 👋 Did you get a chance to review the quote?
AR: مجرد متابعة سريعة 👋 هل قدرت تراجع عرض السعر؟

## 114. Payment Reminder
EN: Friendly reminder: invoice #[INV_NO] of [AMOUNT] BHD is due on [DATE].
AR: تذكير ودي: الفاتورة #[INV_NO] بقيمة [AMOUNT] د.ب مستحقة بتاريخ [DATE].

## 115. Complaint Handling
EN: Sorry for the inconvenience. I will resolve this by [TIME].
AR: نعتذر عن الإزعاج، وسيتم حل الموضوع قبل [TIME].

## 116. Appointment Confirmation
EN: Confirmed ✅ Your booking is on [DATE] at [TIME].
AR: تم التأكيد ✅ حجزك يوم [DATE] الساعة [TIME].

## 117. Reschedule
EN: No problem, we can move your booking to [NEW_DATE] at [NEW_TIME].
AR: أكيد، نقدر نغير الحجز إلى [NEW_DATE] الساعة [NEW_TIME].

## 118. Out of Stock
EN: This item is currently out of stock. Expected availability: [DATE].
AR: المنتج غير متوفر حاليًا. التوفر المتوقع: [DATE].

## 119. Upsell
EN: You may also like [ADDON] for an extra [ADDON_PRICE] BHD.
AR: ممكن يناسبك أيضًا [ADDON] مقابل [ADDON_PRICE] د.ب إضافية.

## 120. Closing
EN: Perfect, I’ll proceed now and keep you updated.
AR: ممتاز، سأبدأ الآن وأوافيك بالتحديثات.
